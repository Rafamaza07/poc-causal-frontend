...
export default function EvaluarPaciente() {
  ...
  const [mapaAjustes, setMapaAjustes] = useState(null)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { data } = await API.post('/api/evaluar-caso', {
        ...
        profesiograma:                form.profesiograma || undefined,
  ...
  const generarMapaAjustes = async () => {
    setMapaLoading(true)
    try {
      const { data } = await API.post('/api/v1/traductor/ajustes-razonables', {
  ...
  if (result) {
    ...
    return (
      <div className="max-w-4xl mx-auto space-y-6 animate-fade-in">
        ...
        {/* Traductor Clínico-Ocupacional — Mapa de Ajustes Razonables */}
        {(form.restricciones_vigentes || form.profesiograma) && (
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 space-y-4">
            <div className="flex items-center gap-2">
              <Stethoscope className="w-5 h-5 text-brand-600" />
              <h2 className="text-sm font-semibold text-gray-800">Mapa de Ajustes Razonables</h2>
              {!mapaAjustes && !mapaLoading && (
                <button
                  onClick={generarMapaAjustes}
                  disabled={!form.restricciones_vigentes || !form.profesiograma}
                  className="ml-auto flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg bg-brand-600 text-white hover:bg-brand-700 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
                >
              ...
              {mapaAjustes && !mapaLoading && (
                <button onClick={() => setMapaAjustes(null)}
            ...
            {!form.profesiograma && !mapaAjustes && (
              <p className="text-xs text-gray-400 italic">
                Completa el profesiograma del cargo en el formulario para habilitar esta función.
              </p>
            )}
            ...
            {mapaAjustes && (
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {mapaAjustes.resumen_compatibilidad && (
                    <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold border ${
                      mapaAjustes.resumen_compatibilidad === 'compatible' ? 'bg-emerald-100 text-emerald-800 border-emerald-300' :
                      mapaAjustes.resumen_compatibilidad === 'parcialmente_compatible' ? 'bg-amber-100 text-amber-800 border-amber-300' :
                      'bg-red-100 text-red-800 border-red-300'
                    }`}>
                      {mapaAjustes.resumen_compatibilidad === 'compatible' ? <CheckCircle className="w-3 h-3" /> :
                       mapaAjustes.resumen_compatibilidad === 'parcialmente_compatible' ? <AlertTriangle className="w-3 h-3" /> :
                       <XCircle className="w-3 h-3" />}
                      {mapaAjustes.resumen_compatibilidad.replace(/_/g, ' ')}
                    </span>
                  )}
                  {mapaAjustes.nivel_riesgo_reintegro && (
                    <span className={`px-2.5 py-1 rounded-full text-xs font-medium border ${
                      mapaAjustes.nivel_riesgo_reintegro === 'bajo' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' :
                      mapaAjustes.nivel_riesgo_reintegro === 'medio' ? 'bg-amber-50 text-amber-700 border-amber-200' :
                      'bg-red-50 text-red-700 border-red-200'
                    }`}>
                      Riesgo {mapaAjustes.nivel_riesgo_reintegro}
                    </span>
                  )}
                </div>

                {/* Recomendación final */}
                {mapaAjustes.recomendacion_final && (
                  <div className="flex gap-2 p-3 bg-brand-50 rounded-lg border border-brand-100">
                    <Clipboard className="w-4 h-4 text-brand-600 shrink-0 mt-0.5" />
                    <p className="text-xs text-brand-900">{mapaAjustes.recomendacion_final}</p>
                  </div>
                )}

                {/* Ajustes */}
                {mapaAjustes.ajustes_recomendados?.length > 0 && (
                  <div>
                    <p className="text-xs font-semibold text-gray-600 mb-2">Ajustes recomendados ({mapaAjustes.ajustes_recomendados.length})</p>
                    <ul className="space-y-1.5">
                      {mapaAjustes.ajustes_recomendados.map((a, i) => (
                        <li key={i} className="flex items-start gap-2 text-xs text-gray-700 bg-gray-50 rounded-lg px-3 py-2">
                          ...
                          <span className="flex-1">{a.ajuste}</span>
                          <span className="ml-1 px-1.5 py-0.5 rounded bg-gray-200 text-gray-500 text-[10px] capitalize shrink-0">{a.urgencia?.replace(/_/g,' ')}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
                ...
                {/* Funciones preservadas vs comprometidas */}
                {(mapaAjustes.funciones_preservadas?.length > 0 || mapaAjustes.funciones_comprometidas?.length > 0) && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {mapaAjustes.funciones_preservadas?.length > 0 && (
                      <div className="p-3 rounded-lg bg-emerald-50 border border-emerald-100">
                        <p className="text-xs font-semibold text-emerald-700 mb-1.5 flex items-center gap-1">
                          <CheckCircle className="w-3 h-3" /> Preservadas
                        </p>
                        <ul className="space-y-1">
                          {mapaAjustes.funciones_preservadas.map((f, i) => (
                            <li key={i} className="text-xs text-emerald-800">• {f}</li>
                          ))}
                        </ul>
                      </div>
                    )}
                    {mapaAjustes.funciones_comprometidas?.length > 0 && (
                      <div className="p-3 rounded-lg bg-red-50 border border-red-100">
                        <p className="text-xs font-semibold text-red-700 mb-1.5 flex items-center gap-1">
                          <XCircle className="w-3 h-3" /> Comprometidas
                        </p>
                        <ul className="space-y-1">
                          {mapaAjustes.funciones_comprometidas.map((f, i) => (
      ...
      {step === 2 && (
        <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm space-y-6">
          ...
          <div className="space-y-5">
            ...
            <Field
              label="Profesiograma del cargo"
              hint="Demandas físicas, postura, cargas, ambiente — necesario para generar Mapa de Ajustes Razonables"
            >
              <textarea value={form.profesiograma}
                onChange={e => set('profesiograma', e.target.value)}
                placeholder="Ej: bipedestación 6–8 h/día, levantamiento 15–20 kg frecuente, trabajo en altura 2–4 m, exposición a vibraciones..."
                className="input w-full resize-none h-24" />
            </Field>
          </div>
